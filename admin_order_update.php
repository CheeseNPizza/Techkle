...
if (isset($_POST['UpdateStatus'])) {
    ...
    $update_status_query = "UPDATE `order` SET status = '$new_status' WHERE order_ID='" . $_REQUEST['order_ID'] . "'";
    mysqli_query($con, $update_status_query) or die(mysqli_error($con));
...
<body>
    ...
    <div class = "profile">
        ...
        <h2>Status: <input type = "text" name = "new_status" value ="<?php $order_query = "SELECT * FROM `order` WHERE order_ID ='". $_REQUEST['order_ID'] . "'";
        $order_result = mysqli_query($con, $order_query) or die ( mysqli_error($con));
        $order_row = mysqli_fetch_assoc($order_result);echo $order_row["status"];?>">
        <input class = "table-btn" type = "submit" name = "UpdateStatus" value = "Update Status"></h2>
    </form>
    </div>
